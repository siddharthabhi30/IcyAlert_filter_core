...
def enkf_update(x_forecast: torch.Tensor, y_obs: torch.Tensor) -> torch.Tensor:
    ...
    # Observation covariance matrix R (2x2)
    R_cov = config.OBS_NOISE_VAR * torch.eye(2, dtype=x_forecast.dtype, device=x_forecast.device)
    obs_std = torch.sqrt(torch.tensor(config.OBS_NOISE_VAR, dtype=x_forecast.dtype, device=x_forecast.device))
    
    # 1. Compute ensemble covariance P
    x_mean = x_forecast.mean(dim=0, keepdim=True)
    anomalies = x_forecast - x_mean
    P = (anomalies.T @ anomalies) / (N - 1)
    
    # 2. Compute Kalman Gain: K = P @ (P + R)^-1
    K = P @ torch.linalg.inv(P + R_cov)
    
    # 3. Generate perturbed observations for each ensemble member
